...
void source_callback(std::uint64_t cid,
                     ...
                     int source_type) {
    source_id = sid;
    std::cout << std::endl << "Got New Source:" << sid << " Client:" << cid
              << " Time:" << ts << " Type:" << source_type << std::endl;
}
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < image_uv_size; i++) {
        image[image_y_size+i*2+0] = (i >> 6 & 0x3f) << 2; // U in bits 6-11
        image[image_y_size+i*2+1] = (i >> 0 & 0x3f) << 2; // V in bits 0-5
    }
    ...
    // Send frames.
    std::thread sendThread([neo, image, image_size, image_width, image_height, enc_format]() {
        while (true)
        {
            ...
            std::cerr << " S "; // << sourceRecordTime << "ns " << image_size << "bytes" << std::endl;
            sendVideoFrame(neo, image, image_size, image_width, image_height,
                           image_width, image_width, image_width*image_height, 0,
                           enc_format, timestamp, send_source_id);
            image[0]=image[0]+1; // Embed a frame counter to check on receive
        }
    });

    // Get frames.
    std::thread recvThread([neo, client_id, image, image_size, image_width, image_height, dec_format]() {
        while (true)
        {
            ...
            auto received_image_size =
                   getVideoFrame(neo, client_id, source_id, ts,
            ...
            // Enable assert below if testing Raw video with echoSFU and no concealment.
            // Must explicitly disable AV1 and decoder concealment for this assert to pass.
            // assert(buffer[0] == 0x80 || memcmp(buffer, image, image_size) == 0);
            std::cerr << " " << (int)(buffer[0]) << " "; // log frame counter

            /* PSNR calc to test sanity (30-50 is believable) and quality (>40=great)
            std::int64_t sq_err = 1;
            for (int i = 0; i < image_size; ++i) {
                int diff = image[i] - buffer[i];
                sq_err += diff * diff;
            }
