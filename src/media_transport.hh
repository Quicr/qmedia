...
struct MediaTransport : TransportMessageHandler {
    ...
    virtual void send_data(uint64_t id, quicr::bytes &&data, uint8_t priority, uint64_t group_id, uint64_t object_id) = 0;
...
struct QuicRMediaTransport : public MediaTransport
{
    ...
    virtual void send_data(uint64_t id, quicr::bytes &&data, uint8_t priority, uint64_t group_id, uint64_t object_id) override;
...
private:
    ...
    bool shutdown = false;
    bool enable_media_priority = false;
};
